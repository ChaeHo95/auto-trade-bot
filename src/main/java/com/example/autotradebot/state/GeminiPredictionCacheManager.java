...
@Component
public class GeminiPredictionCacheManager {

    @Value("${symbols}")
    private List<String> symbols;

    private final ConcurrentHashMap<String, List<PredictionDTO>> geminiPredictionCache = new ConcurrentHashMap<>();

    private MarketAnalysisMapper marketAnalysisMapper;

    @Autowired
    private GeminiPredictionCacheManager(MarketAnalysisMapper marketAnalysisMapper) {
    ...
    public void init() {
        for (String symbol : symbols) {
            List<PredictionDTO> predictionDTOList = marketAnalysisMapper.getChartAnalysisLimit("GEMINI", symbol, 200);
    ...
    public void putPrediction(String symbol, PredictionDTO predictionDTO) {
        geminiPredictionCache.computeIfAbsent(symbol, k -> new ArrayList<>()).add(predictionDTO);
        removeOldEntries(symbol);
    }

    // ✅ 가장 최신 데이터 반환 (없으면 null)
    public PredictionDTO getLatestPrediction(String symbol) {
        List<PredictionDTO> predictionList = geminiPredictionCache.get(symbol);
    ...
    public List<PredictionDTO> getAllPredictions(String symbol) {
        return geminiPredictionCache.getOrDefault(symbol, new ArrayList<>());
    }
    ...
    public void removePrediction(String symbol) {
        geminiPredictionCache.remove(symbol);
    }
}
